...
import serial, struct, sys, time, json, platform, os
...
if __name__ == "__main__":
    ...
    while True:
        ...
        pm25 = values[0]
        pm10 = values[1]
        outLine = '{} UTC,{},{}'.format(time.strftime("%d.%m.%Y %H:%M:%S"), pm25, pm10)
        dataFileName = time.strftime('data_'+host+'_%Y_%b_%d.csv')
        dataFile = os.path.join("/var/www/html/airquality", dataFileName)
        ...
        print("Going to sleep for 32 sec...")
        cmd_set_mode(0);
        cmd_set_sleep()
        time.sleep(32)
